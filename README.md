# Telegram Number Checker API — Bulk Verification | NumberChecker

The **Telegram Number Checker API** accepts a text file of phone numbers, creates an asynchronous bulk task, and exports a result file. This repository contains a runnable integration in eight languages.

- **Product page:** https://numberchecker.ai/products/telegram-checker
- **API documentation:** https://docs.numberchecker.ai/telegram-bulk-number-checker
- **API base URL:** `https://api.numberchecker.ai`
- **Authentication:** `X-API-Key`
- **Get an API key:** https://platform.numberchecker.ai

> [!NOTE]
> This repository covers the standalone product code `tg` only. Other product codes are outside its scope.
...
The current catalog notes: Live sample currently contains only number and activated; username/member are not present.

## Workflow

1. Submit the input file to `POST /v1/tasks` with `task_type=tg`.
...
Only `tg`. The task type is intentionally fixed to that product code.
...
- **Product:** https://numberchecker.ai/products/telegram-checker
- **Documentation:** https://docs.numberchecker.ai/telegram-bulk-number-checker
